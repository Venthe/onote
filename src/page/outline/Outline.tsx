    ...
    return (
        <>
            <div className="outline__position-origin outline" data-v-position={id}
                 style={{pointerEvents: currentlyEditing && !dragged && !editable && !resized ? "none" : "auto"}}>

                <div onMouseEnter={handleHoverIn} onMouseLeave={handleHoverOut}
                     className={`outline__container ${editable && "outline__container--selected"} ${showOutline && "outline__container--hover"}`}>
                    <div onMouseDown={startDrag}
                         className={`outline__title-container ${editable && "outline__title-container--selected"} ${showOutline && "outline__title-container--hover"}`}>
                        <span onMouseDown={startResize}
                              className={`outline__resize-handle ${editable && "outline__resize-handle--selected"} ${showOutline && "outline__resize-handle--hover"}`}/>
                    </div>
                    <div onMouseDown={startResize}
                         className={`outline__resize ${debug && "outline__resize--debug"}`}></div>
...
const useSizingAndPosition = (props: {
    commit?: OnCommit,
    ...
    const startResize: MouseEventHandler<any> = useCallback((e) => {
        e.stopPropagation();
        e.preventDefault();
        setResized(true)
    }, [])
    const startDrag = useCallback((e: React.MouseEvent<HTMLDivElement, MouseEvent>): void => {
        e.stopPropagation();
        e.preventDefault();
        setDragged(true);
    }, []);
...
const DebugInfo = (props: {
    ...
    isResized: boolean,
    type?: string,
    box: { width: number, left: number, top: number },
    className?: string
