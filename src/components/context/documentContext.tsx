...
const notInitialized = (name: string) => () => { throw new Error(`${name} not initialized`) }
...
export const DocumentContextProvider = (props: PropsWithChildren<{ page?: IPage, onCommit?: (page: IPage) => void }>) => {
  ...
  const dirtyOutlineCommands = useMemo(() => ({ commit, dirtyChange, startEdit, createOutline }), [props.page, props.onCommit])
